...
<div class="min-h-screen flex items-start justify-center bg-gradient-to-br from-emerald-900 via-green-800 to-lime-700 px-4 relative overflow-hidden py-20">
  ...
  <div class="relative w-full max-w-md bg-white/10 backdrop-blur-md border border-white/20 shadow-2xl rounded-3xl p-8 animate-fadeIn z-10 mt-6">
    ...
    <div class="text-center mb-6">
      ...
      <p class="text-emerald-100 text-sm mt-1">Buat akun untuk mengakses layanan publik MAN Ambon</p>
    </div>
    ...
    {{-- Form --}}
    <form action="{{ route('register.masyarakat.post') }}" method="POST" class="space-y-4 relative">
      @csrf
      ...
      <div class="form-control">
        ...
        <div class="relative flex items-center">
          ...
          <input type="email" name="email" class="input input-bordered w-full h-12 pl-10 rounded-xl bg-white/20 text-white placeholder-gray-200 border-none focus:ring-2 focus:ring-yellow-400 transition-all" required placeholder="contoh@email.com">
        </div>
      </div>
      ...
      {{-- NIK (opsional) --}}
      <div class="form-control">
        <label class="label font-semibold text-sm text-emerald-100">NIK (Opsional)</label>
        <div class="relative flex items-center">
          <i class="fa-solid fa-id-card absolute left-4 text-emerald-400 text-lg"></i>
          <input type="text" name="nik" class="input input-bordered w-full h-12 pl-10 rounded-xl bg-white/20 text-white placeholder-gray-200 border-none focus:ring-2 focus:ring-yellow-400 transition-all" placeholder="Masukkan NIK (jika ada)">
        </div>
      </div>
    ...
    <div class="text-center mt-6">
      ...
      <a href="{{ route('login.masyarakat.login-masyarakat') }}" class="text-yellow-300 font-semibold hover:text-white transition-all duration-200">
        Kembali ke Login
      </a>
    </div>
  </div>
